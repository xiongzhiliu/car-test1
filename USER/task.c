...
void task1(void)
{
    ...
    case 3:
    {
        gray_flag = 0;        // 循环状态 1：识别终点  2：识别到黑复合道路
        if (NODE_DETECT_FLAG) // 发现 节点
        {
            ...
            // INFRARED_ERROR = 0;
            error = 0;
            delay_ms(100);  // 往前走 //可能增加一个延时计数，定时器使用
...
int add_point(float get_y, float get_x, int get_left, int get_qian, int get_right, int get_hou)
{
    ...
    // judge_dir();
    if (go == &save)
    { // save是map结构初始化的结构体，相当于go==一个初始化节点，且未记录的节点时
        // printf("save\r\n");
        p_point = 0;
        // 遍历所有的点，检查是否存在相同坐标的点
            ...
            { // get_x 在当前点范围内
                ...
                { // 在某点的范围内
                    ...
                    {                          // p_t为当前点的标号 找到p_point（点在检索点中）
                        ...
                        { // 判断上一次的转向，根据上次的转向，记录当前点（重复点）与上一个点的链接关系
                        case 1:
                            last->next_left = now;
                            now->next_right = last;
