// 此软件I2C由 @classmateliu 负责维护
// 软件I2C驱动 OLED显示 由标准库转换为HAL库

#include "stm32f1xx_hal.h"   // HAL库主头文件
#include "OLED.h"            // 自定义模块
#include <string.h>
#include <math.h>
#include <stdio.h>
#include <stdarg.h>
...
// 此处首先定义全局变量=========================================================================
  ...
  */
uint8_t OLED_DisplayBuf[8][128];  // OLED显示缓冲区，分8页，每页128列（128x64像素）
...
void OLED_GPIO_Init(void)
{
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_4;                    // SDA
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = GPIO_PIN_5;                    // SCL
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
...
void OLED_Init(void)
{
    ...
    OLED_WriteCommand(0x80);  // 建议值：0x80

    OLED_WriteCommand(0xA8);  // 设置多路复用率
    OLED_WriteCommand(0x3F);  // 64行驱动器，对应0x3F

    OLED_WriteCommand(0xD3);  // 设置显示偏移
    OLED_WriteCommand(0x00);  // 通常设为0

    OLED_WriteCommand(0x40);  // 设置显示开始行（从0开始）

    OLED_WriteCommand(0xA1);  // 设置左右方向，0xA1正常，0xA0左右反置
    OLED_WriteCommand(0xC8);  // 设置上下方向，0xC8正常，0xC0上下反置

    OLED_WriteCommand(0xDA);  // 设置COM引脚硬件配置
    OLED_WriteCommand(0x12);  // 建议值：0x12

    OLED_WriteCommand(0x81);  // 设置对比度控制
    OLED_WriteCommand(0xCF);  // 亮度范围：0x00~0xFF，建议0xCF

    OLED_WriteCommand(0xD9);  // 设置预充电周期
    OLED_WriteCommand(0xF1);  // 建议值：0xF1

    OLED_WriteCommand(0xDB);  // 设置VCOMH电压
    OLED_WriteCommand(0x30);  // 建议值：0x30

    OLED_WriteCommand(0xA4);  // 设置整个显示打开/关闭（0xA4=跟随RAM，0xA5=全部点亮）

    OLED_WriteCommand(0xA6);  // 设置正常/反色显示，0xA6正常，0xA7反色

    OLED_WriteCommand(0x8D);  // 设置充电泵
    OLED_WriteCommand(0x14);  // 开启充电泵（0x14）

    OLED_WriteCommand(0xAF);  // 开启显示（0xAF）
}
...
void OLED_SetCursor(uint8_t Page, uint8_t X)
{
    ...
    /* 因为1.3寸的OLED驱动芯片（SH1106）有132列，屏幕起始列为2 */
    // X += 2;

    /* 通过指令设置页地址和列地址 */
    OLED_WriteCommand(0xB0 | Page);                 // 设置页位置（0~7）
